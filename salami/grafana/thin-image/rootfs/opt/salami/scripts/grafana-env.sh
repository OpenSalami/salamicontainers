...
# 4. Environment variables set externally (i.e. current shell context/Dockerfile/userdata)
...
export BITNAMI_VOLUME_DIR="/salami"
...
for env_var in $grafana_env_vars; do
    ...
    eval file_env_path=\${$file_env_var:-}
    if [ -n "$file_env_path" ]; then
        if [ -r "$file_env_path" ]; then
            eval "export $env_var=\"\$(cat \"\$file_env_path\")\""
            unset "$file_env_var"
        else
...
# Grafana paths
export GRAFANA_BASE_DIR="/grafana"
